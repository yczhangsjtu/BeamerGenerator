...
#        self.tree.Expand(self.rootID)
    ...
    def buildJSONTree(self, jsonObj, parentID, parentTitle = None):
        if jsonObj == None:
        ...
        if isinstance(jsonObj, dict):
            for (k,v) in jsonObj.iteritems():
                ...
                self.buildJSONTree(v, childID, k.encode('utf8'))
#                self.tree.Expand(childID)
        elif isinstance(jsonObj, list):
            for index,item in enumerate(jsonObj):
                if parentTitle:
                    childID = self.tree.AppendItem(parentID, '%s[%d]'%(parentTitle,index))
                else:
                    childID = self.tree.AppendItem(parentID, '[%d]'%(index))
